...
function south_boatman_main(BOATID)
	...
	if (Fengling_ferry ~= 1) then
	return Talk(1,"","Tİnh N¨ng Phong L¨ng §é §· §­îc T¹m §ãng Vui Lßng \nQuay L¹i Sau")
	end
	local MemberNowDate = tonumber(GetLocalDate("%y%m%d"));
	if(GetTask(5999) ~= MemberNowDate) then
		SetTask(5998,Fengling_res);  
		SetTask(5999,MemberNowDate);	
	end
	
	
	if GetLevel() < Fengling_Lv then
		return Talk(1,"","Yªu CÇu §¼ng CÊp Tõ <color=yellow>"..Fengling_Lv.."<color> Trë Lªn Míi Cã ThÓ Tham Gia   Tİnh N¨ng <color=yellow>Phong L¨ng §é<color>")
	end
