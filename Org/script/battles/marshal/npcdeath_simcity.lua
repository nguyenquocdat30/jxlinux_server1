...
function OnDeath(nNpcIndex)
	...
	local currank = GetNpcParam(nNpcIndex, PARAM_TYPE)
	--local attackerIndex
	...
	dropItem(nNpcIndex, currank, PlayerIndex)
	...
	if not playerInTK[nW] then
		playerInTK[nW] = {}
	end
	playerInTK[nW][PlayerIndex] = {
		phe = "T",
		rank = launrankname,
...
function dropItem(nNpcIndex, nNpcRank, nBelongPlayerIdx)
	...
	--local nNpcRank = GetNpcParam(nNpcIndex, PARAM_TYPE)
	for nDropTimes = 1, NPC_RANK_DROPRATE_TABLE[nNpcRank] do
		...
		for i = 1, nItemCount do
			...
			if (nSum > nRandNum) then
				DropItem(nSubWorldIdx, nMpsX, nMpsY, nBelongPlayerIdx, ITEM_DROPRATE_TABLE[i][1][1],
					ITEM_DROPRATE_TABLE[i][1][2], ITEM_DROPRATE_TABLE[i][1][3],
					ITEM_DROPRATE_TABLE[i][1][4], ITEM_DROPRATE_TABLE[i][1][5],
					ITEM_DROPRATE_TABLE[i][1][6], ITEM_DROPRATE_TABLE[i][1][7]);
				break
			end
		end
	end
