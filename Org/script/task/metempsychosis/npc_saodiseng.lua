...
function zhuansheng_ondialog()
	...
	if (n_transcount == 0 and n_level == 200 and (n_date >= ZHUANSHENG_ITEM_BEGIN and n_date <= ZHUANSHENG_ITEM_ENDLE)) then
...
function zhuansheng_want_learn()
	...
	if (n_transcount >= 5) then
		CreateTaskSay({"<dec><npc>"..format("ChØ cã thÓ Trïng Sinh tèi ®a %d lÇn", 5), 
...
function check_zhuansheng()
	...
	if tbPVLB_Check:IsNewPlayer() == 1 and tbPVLB_Check:CheckTime() == 1 then
		...
		elseif ntranscount == 1 then
			...
			if (ntranscount < 3 and GetCash() < ZHUANSHENG_TUITION) then
...
function zhuansheng_surexiandan()
	...
	if (n_curexp - ZHUANSHENG_XIANDAN_MINEXP >= ZHUANSHENG_XIANDAN_BASEEXP) then
		...
		if (nitemidx > 0) then
			...
			ITEM_SetExpiredTime(nitemidx, ZHUANSHENG_ITEM_EXTIME)
			SyncItem(nitemidx)
