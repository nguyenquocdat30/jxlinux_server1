...
local tb = tbEquipTryOn:NewTemplate("KiÕm")

tb.szFile_Male = "\\settings\\changefeature\\goldequip_male_kiem.txt"
tb.szFile_Female = "\\settings\\changefeature\\goldequip_female_kiem.txt"
...
function tb:CheckEquip(nItemIndex)
	...
	if nD ~= 0 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i KiÕm.")
		return 
	end
	if nP ~= 0 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i KiÕm.")
