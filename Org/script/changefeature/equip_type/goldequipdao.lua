...
local tb = tbEquipTryOn:NewTemplate("§¬n §ao")

tb.szFile_Male = "\\settings\\changefeature\\goldequip_male_dao.txt"
tb.szFile_Female = "\\settings\\changefeature\\goldequip_female_dao.txt"
...
function tb:CheckEquip(nItemIndex)
	...
	if nD ~= 0 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i §¬n §ao.")
		return 
	end
	if nP ~= 1 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i §¬n §ao.")
