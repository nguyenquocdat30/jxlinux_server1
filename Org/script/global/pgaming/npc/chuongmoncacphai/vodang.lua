...
function oldentence()
	...
	if (UTask_wd == 70*256) and (GetFaction() == "wudang") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_wd == 70*256) and (GetTask(9) >= 5*256) and (GetTask(9) < 10*256) then
		SetTask(9,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_wd == 80*256) and (GetCamp() == 4) then
		SetTask(5,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	...
	local tbDes = {
		"N©ng cÊp Trang BŞ TuyÖt §Ønh Vâ §ang/#tbHKUpgrade:VoDang()",
		"N©ng cÊp Ên TuyÖt §Ønh Vâ §ang/#tbAnUpgrade:VoDang()",
		"§¸i nghÖ ®Çu s­/#daiyitoushi_main(8)",
...
function common_talk()
	...
	elseif (GetSeries() == 4) and (GetFaction() == "wudang") then
		...
		elseif (UTask_wd == 60*256) and (GetLevel() >= 50) then
		...
		elseif ((UTask_wd == 40*256+60) or (UTask_wd == 40*256+80)) and (HaveItem(164) == 1) then
		...
		elseif (UTask_wd == 10*256+70)  then
			L10_Q3()
		elseif (UTask_wd == 80*256) then
			check_skill()
		elseif (UTask_wd == 10*256+20) then
			Talk(1,"","§Ö tö Vâ §ang ph¸i ta ®Òu ph¶i häc thuéc <color=Red>'§¹o §øc Kinh'<color>. Ng­¬i h·y ®Õn thØnh gi¸o <color=Red>DiÖp TiÕp Mü<color> s­ thóc, häc thuéc'§¹o §øc Kinh'. Sau ®ã quay l¹i l¹i t×m ta")
		elseif (UTask_wd > 60*256) and (UTask_wd < 70*256) then
		...
		elseif (UTask_wd >= 50*256) and (GetFaction() == "wudang") then
	...
	elseif (GetSeries() == 4) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_wd == 70*256) and (GetTask(9) < 5*256) then
...
function defection_yes()
	n = 0
	i=153 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=151 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=152 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=155 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=156 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=157 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=158 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=159 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=160 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=161 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=162 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=164 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=165 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=166 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
