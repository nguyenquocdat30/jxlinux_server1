...
function HoTroKyNang()
SupportLearnSkills:Give()
end


function SupportLearnSkills:Give()
	...
	if not (TAB_SKILLALL[nFaction]) then
		Talk(1,"","B¹n ch­a gia nhËp m«n ph¸i kh«ng thÓ nhËn kü n¨ng m«n ph¸i!")
	return end
	
	--*****	Chøc n¨ng nhËn kü n¨ng cÊp 1x->7x m«n ph¸i	*****--
	if (TAB_SKILLALL[nFaction].tbSkillBase) then
		if (getn(TAB_SKILLALL[nFaction].tbSkillBase) > 0) then
			for i = 1, getn(TAB_SKILLALL[nFaction].tbSkillBase) do
				if (HaveMagic(TAB_SKILLALL[nFaction].tbSkillBase[i]) == -1) then
					...
					Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(TAB_SKILLALL[nFaction].tbSkillBase[i])))
				end
			end
		end
	end
	
	--*****	Chøc n¨ng nhËn kü n¨ng cÊp 90 m«n ph¸i	*****--
	if (TAB_SKILLALL[nFaction].tbSkill9x) then
		if (getn(TAB_SKILLALL[nFaction].tbSkill9x) > 0) then
			for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill9x) do
				if (self:SkillMain(TAB_SKILLALL[nFaction].tbSkill9x[i]) == 1) then
					if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill9x[i]) == -1) then
						AddMagic(TAB_SKILLALL[nFaction].tbSkill9x[i])
						Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(TAB_SKILLALL[nFaction].tbSkill9x[i])))
					end
				else
					if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill9x[i]) == -1) then
	...
	--*****	Chøc n¨ng nhËn kü n¨ng cÊp 120 m«n ph¸i	*****--
	if (TAB_SKILLALL[nFaction].tbSkill12x) then
		if (getn(TAB_SKILLALL[nFaction].tbSkill12x) > 0) then
			for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill12x) do
				if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill12x[i]) == -1) then
					...
					Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(TAB_SKILLALL[nFaction].tbSkill12x[i])))
				end
			end
		end
	end
	
	--*****	Chøc n¨ng nhËn kü n¨ng cÊp 150 m«n ph¸i	*****--
	--if (TAB_SKILLALL[nFaction].tbSkill15x) then
		--if (getn(TAB_SKILLALL[nFaction].tbSkill15x) > 0) then
			--for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill15x) do
				--if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill15x[i]) == -1) then
					...
					--Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(TAB_SKILLALL[nFaction].tbSkill15x[i])))
				--end
			--end
			--SetTask(TSK_LV150_SKILL, SBOOK_150SKILL_TASKSTATE*100+sbook_factstep_list[nFaction])
		--end
	--end
	
	--*****	Chøc n¨ng nhËn kü n¨ng cÊp 180 m«n ph¸i	*****--
	--if (TAB_SKILLALL[nFaction].tbSkill18x) then
		--if (getn(TAB_SKILLALL[nFaction].tbSkill18x) > 0) then
			--for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill18x) do
				--if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill18x[i]) == -1) then
					--AddMagic(TAB_SKILLALL[nFaction].tbSkill18x[i],20)
					--Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(TAB_SKILLALL[nFaction].tbSkill18x[i])))
				--end
			--end
		--end
	--end
	...
	--if (HaveMagic(210) == -1) then
		--AddMagic(210,1)
		---Msg2Player(format("<color=earth>B¹n ®· lÜnh ngé ®­îc kü n¨ng %s!<color>",GetSkillName(210)))
	--end
end

function SupportLearnSkills:SkillMain(nSkillID)
