...
Include("\\script\\global\\zid_jx\\global\\fixscriptnpc.lua");
Include("\\script\\global\\zid_jx\\lib\\config.lua")
Include("\\script\\global\\zid_jx\\global\\auto_addnpc.lua")
Include("\\script\\global\\zid_jx\\tinhnang\\hotonghuyetthu\\addnpc_huyetthu.lua")
...
adddialognpc = {
	...
	{377,387,1570,3132,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,388,1570,3132,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,389,1570,3132,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},

	{377,390,1320,3185,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,391,1320,3185,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,392,1320,3185,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},

	{377,393,1412,3203,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,394,1412,3203,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
	{377,395,1412,3203,"\\script\\global\\ËÍĞÅÈÎÎñ.lua","DŞch quan"},
...
local tbActNpcList =
{
	--{311,176,1593,2943,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,11,3108,5060,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,1,1585,3230,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,37,1740,3065,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,162,1613,3190,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,78,1608,3238,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	--{311,80,1704,3058,"\\script\\missions\\dungeon\\dungeons\\shiliantang\\jieyinren.lua","Ng­êi tiÕp ®ãn thİ luyÖn ®­êng"},
	...
	--{1576, 176,1408,3309,"\\script\\activitysys\\npcdailog.lua","Nam Cung NhÊt Thiªn"},		-- ÎäÁÖÃËÖ÷µñÏñ
...
--ÔÂ¸èµº
	...
	if Server_Open_Test == 1 then
		AddNpc_Zid(NpcThuNghiemTab)
	else
		AddNpc_Zid(TanThuQuanTab)
	end

	buildAllCityInfoLeague()
	tbAloneShipped:AddNpcDialog()
	-- ¹Ò»úµØÍ¼ÈÎÎñ
	load_mission_aexp();
	add_dialognpc(adddialognpc);
	add_dialognpc(nw_npclist)	-- Tİnh n¨ng Thiªn Tö **blackcupid**
	add_dialognpc(npclist_sevencity)
	add_newtasknpc(addnewtasknpc);	-- quest míi Long Ngu Nhiem vu hoang kim
	add_xishancunnpc(xishancunnpc);
	add_spreadernpc(spreadernpc);		--¼ÓÔØÓÎÏ·ÍÆ¹ãÔ±
	if KillerBoss == 1 then
	...
	wlls_autoexe()				--THEM NPC LIEN DAU wlls_autoexec.lua
  	--Á½ÖÜÄê»î¶¯NPC    
	add_dialognpc(%tbActNpcList)	--THEM NPC tbActNpcList   autoexec.lua
	tongwar_addsignnpc()
	add_tongnpc()		--THEM NPC addtongnpc.lua
	...
	if (szRegion =="vn") then
		add_dialognpc(ACT2YEAR_YN_NPC);		--THEM NPC SU GIA VO LAM \script\event\act2years_yn\head.lua
		AnonymVale_auto();				--VO DANH COC judgeoffline_special.lua
		--------------------------------------------------------------------------------------------------------------------------------------
		add_dialognpc(addmasknpc)			--THEM NPC autoexec.lua
		--------------------------------------------------------------------------------------------------------------------------------------
		add_dialognpc(tab_springfestivalNPC);
		add_dialognpc(addmoontown_mulao);	--THEM NPC Môc L·o autoaddnpc.lua
		--add_dialognpc(tbxmas2007_girl_binger) ----THEM NPC B¨ng Nhi  \script\event\vnchristmas2007\addnpc.lua
		--add_dialognpc(newyear_2008_chunjiecailu) ----THEM NPC C©y §µo C©y Mai \script\event\newyear_2008\cailu\addnpc.lua¯
		--Î÷É½Óì
		add_dialognpc(tbaddJinShanDao_NpcAndTrap.tbDialogNpc);--npc --addnpcandtrap.lua
		tbaddJinShanDao_NpcAndTrap:AddTrapR()---addnpcandtrap.lua
		tbaddJinShanDao_NpcAndTrap:AddTrapL()---addnpcandtrap.lua
		tbaddJinShanDao_NpcAndTrap:AddMons()---addnpcandtrap.lua
		--add_obstacle_map();
		if (n_date <= 20200101) then			-- THOI GIAN HOAT DONG JxWeb
		add_dialognpc(tab_zingplay_npc);	--Them NPC Sø Gi¶ Ho¹t §éng autoexec.lua
		end
		...
		tbChangeDestiny:addNpc();			-- \script\event\change_destiny\npc.lua
		tbNewPracticeMap:addNpc();			-- \script\missions\newpracticemap\npc.lua
		tbBeatTiger:AddNpc();			-- \script\event\jiefang_jieri\201004\beat_tiger\head.lua
	end
	--Ôö¼Ó´óÀíºÚ¶´trapµã
	add_trap_daliheidong()				--autoexec.lua				
	--Ôö¼Óãê¾©Î÷³ÇÃÅtrapµã
	add_trap_bianjingximen()				--bianjing_ximen_trap.lua
	add_heisha_entertrap();				--heisha_autoexec.lua
