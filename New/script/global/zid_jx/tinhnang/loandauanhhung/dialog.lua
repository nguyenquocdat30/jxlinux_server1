...
function about(nStep)
	...
	local tbOpt = {}	
	if nStep == 0 then
		szTitle = "<npc>Thêi gian ho¹t ®éng mçi ngµy vµo lóc 20:00, tÊt c¶ ng­êi ch¬i cÊp 80 trë lªn cã thÓ ®Õn b¸o danh víi ta, thêi gian b¸o danh lµ 10 phót, b¸o danh cÇn ph¶i giao nép 10 v¹n l­îng. Sau khi ho¹t ®éng b¾t ®Çu cã thÓ sö dông kü n¨ng cña b¶n th©n ®Ó tham chiÕn, thêi gian lo¹n chiÕn lµ 30 phót, mçi mét ng­êi ®Òu cã 5 c¬ héi phôc sinh, sau khi ho¹t ®éng kÕt thóc c¨n cø vµo ®iÓm tÝch lòy ®Ó nhËn th­ëng."
		tinsert(tbOpt, {"Lo¹n §Êu Anh Hïng lµ mét n¬i nh­ thÕ nµo", about, {1}})
		tinsert(tbOpt, {"Cã phÇn th­ëng g× kh«ng", about, {2}})
	elseif nStep == 1 then
		szTitle = "<npc>Lo¹n §Êu Anh Hïng lµ mét n¬i bÝ mËt mµ mÊy n¨m gÇn ®©y triÒu ®×nh dïng ®Ó huÊn luyÖn nh÷ng ®¹i néi cao thñ. §Õn phót thø 30 nÕu nh­ kh«ng ph©n th¾ng b¹i t×m ra ng­êi cuèi cïng Th× SÏ Chän Ng­êi Cã Sè H¹ Gôc §èi Thñ NhiÒu NhÊt."
...
function round_award()
	...
	if not NewGame.Rank then
		return
	end
	local szCurName = GetName()
	if NewGame.Scores[szCurName] == nil or NewGame.Scores[szCurName] == 0 then
		return Talk(1, "", "Ng­¬i vÉn ch­a tham gia Lo¹n ChiÕn Anh Hïng")
	end
	
	if NewGame.Scores[szCurName] < 0 then
		return Talk(1, "", "§¹i hiÖp ®· nhËn phÇn th­ëng nµy råi.")
	end
	local nCount = getn(NewGame.Rank)
	if nCount > MAX_AWARD_COUNT then
		nCount = MAX_AWARD_COUNT
	end
	local nRank = nil
	for i=1, nCount do
		local szName = NewGame.Rank[i][1]
	...
	if nRank then
		...
		local nAwardCount = getn(LDAHRankAward)
		local nExp = LDAHRankAward[nRank]
		if nRank > nAwardCount then
			nExp = LDAHRankAward[nAwardCount]
		end
		if nExp then
			tbAwardTemplet:Give({nExp_tl = nExp}, 1, {"Lo¹n ChiÕn Anh Hïng","PhÇn th­ëng cè ®Þnh"})
			NewGame.Scores[szCurName] = -1
		end
	else
...
function last_man_award()
	...
	if LastMan and LastMan.szName and LastMan.szName == GetName() and LastMan.bFlag == nil then
		...
		tbAwardTemplet:Give(LastManAward, 1, {"Lo¹n ChiÕn Anh Hïng","PhÇn th­ëng cña dòng sü cuèi cïng"})
		LastMan.bFlag = 1
	else
...
function final_award()
	...
	if nRank and TotalRankAward[nRank] then
		tbAwardTemplet:Give(TotalRankAward[nRank], 1, {"Lo¹n ChiÕn Anh Hïng","PhÇn th­ëng cña dòng sü cuèi cïng"})
...
function give_award()
	...
	local tbOpt = 
	{
		...
		{"Ta ®Õn nhËn th­ëng phÇn th­ëng cña [dòng sü cuèi cïng]" ,last_man_award},
...
function join_LDAH()
	...
	if (ST_GetTransLifeCount() <= 0 and GetLevel() < 80) then
		return Msg2Player(format("CÊp ph¶i ®¹t ®Õn <color=yellow>%d<color>.", 80))
	end
	
	if PlayerFunLib:GetTaskDailyCount(2710) >= 1 then
		if (CalcEquiproomItemCount(6, 1, 4259, -1) < 1) then 
			Talk(1, "", "Ng­¬i ®· tham gia mét lÇn miÔn phÝ råi, lÇn nµy tham gia cÇn ph¶i cã 1<color=yellow> LÖnh Bµi Lo¹n ChiÕn Anh Hïng Th¹ch")
			return
		else
			ConsumeEquiproomItem(1, 6, 1, 4259, -1)
			Talk(1, "", "Ng­¬i ®· Dïng 1 <color=yellow>LÖnh Bµi Lo¹n ChiÕn Anh Hïng Th¹ch<color>, §Ó Tham Gia LÇn TØ vâ LÇn Nµy !")
		end
	end
	if PlayerFunLib:GetTaskDailyCount(2710) < 1 then
	...
	pDungeon:GotoNewGame()
	if PlayerFunLib:GetTaskDailyCount(2710) == 1 then
		tbLog:PlayerActionLog("TinhNangKey","BaoDanhLoanChienAnhHungMienPhi")
	else
		tbLog:PlayerActionLog("TinhNangKey","BaoDanhLoanChienAnhHungThuPhi")
	end
end
function main()
if HeroesOfWar ~= 1 then
	return Talk(1,"","TÝnh N¨ng <color=yellow>Lo¹n ChiÕn Anh Hïng<color>§ang T¹m §ãng Vui Lßng  Quay l¹i Sau")
end
dialog_main()
end
function dialog_main()
	...
	local tbOpt = 
	{
		{"Liªn quan ®Õn Lo¹n ChiÕn Anh Hïng", about, {0}},
	...
	if pDungeon then
		if pDungeon.nState == 1 then
			tinsert(tbOpt, 1, {"B¸o danh tham gia Lo¹n ChiÕn Anh Hïng", join_LDAH, {}})
		end
	end
