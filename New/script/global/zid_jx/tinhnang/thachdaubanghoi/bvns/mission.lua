...
-------------------------------------------------------------------------
	...
	StartMissionTimer(MISSIONID, TOTALTIMER, TIMER_2);
	--Ëæ»úÄ£Ê½ÐèÒª´ò¿ªÃ¿X·ÖÖÓ´¥·¢Ò»´ÎµÄ´¥·¢Æ÷£¬ÒÔ²úÉúÐÂÆì
	StartMissionTimer(MISSIONID, SMALLTIMER, TIMER_1);
	
	SetMissionV(MS_STATE, 1);--ÔÊÐí±¨ÃûÁË

	RestMin, RestSec = GetMinAndSec(600);
	local str1 = "Tèng Kim (B¶o vÖ Nguyªn So¸i) ®ang trong giai ®o¹n b¸o danh. C¸c hiÖp kh¸ch muèn tham gia h·y nhanh chãng ®Õn T­¬ng D­¬ng hoÆc Chu Tiªn trÊn ®Ó b¸o danh! (hoÆc dïng Tèng Kim Chiªu th­) .Thêi gian b¸o danh cßn l¹i lµ::"..RestMin.."phót"..RestSec.." gi©y. §iÒu kiÖn tham gia: CÊp kh«ng Ýt h¬n 40, phÝ b¸o danh 100000 l­îng";
	--AddGlobalNews(str1);
	CreateChannel("Phe Tèng "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i", 9)
	CreateChannel("Phe Kim "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i", 10)
...
function EndMission()
	...
	StopMissionTimer(MISSIONID, SMALLTIMER);
	StopMissionTimer(MISSIONID ,TOTALTIMER);
	
	GameOver()
	DeleteChannel("Phe Tèng "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i")
	DeleteChannel("Phe Kim "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i")
...
function OnLeave(RoleIndex)
	...
	if (GetCurCamp() == 1) then
		LeaveChannel(PlayerIndex, "Phe Tèng "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i")
	else
		LeaveChannel(PlayerIndex, "Phe Kim "..szGAME_GAMELEVEL.." B¶o vÖ Nguyªn so¸i")
	end
	sf_onplayerleave()
