...
function skills_update()
	...
	elseif (Player_Faction == 9) then		-- À¥ÂØ
		update_kl()
	elseif (Player_Faction == 3481) then		-- À¥ÂØ
		update_hs()
	elseif (Player_Faction == 4406) then		-- À¥ÂØ
		update_wh()
	elseif (Player_Faction == 4464) then		-- À¥ÂØ
		update_xy()
...
function check_faction()
	...
	elseif (GetTask(5) >= 10*256) and (GetTask(5) ~= 75*256) then		-- Îäµ±»ò´ËÃÅÅÉ³öÊ¦
		...
		return 5
	elseif (GetTask(3481) >= 10*256) and (GetTask(3481) ~= 75*256) then		-- Îäµ±»ò´ËÃÅÅÉ³öÊ¦
		SetTask(137,3481)
		SetLastFactionNumber(10)
		return 3481
	elseif (GetTask(4406) >= 10*256) and (GetTask(4406) ~= 75*256) then		-- Îäµ±»ò´ËÃÅÅÉ³öÊ¦
		SetTask(137,4406)
		SetLastFactionNumber(11)
		return 4406
	elseif (GetTask(4464) >= 10*256) and (GetTask(4464) ~= 75*256) then		-- Îäµ±»ò´ËÃÅÅÉ³öÊ¦
		SetTask(137,4464)
...
function update_hs()			-- Ë¢ĞÂ¼¼ÄÜ
	UTask = GetTask(3481)
	if (GetTask(3486) == 255) then		-- Ñ§»áÁË90¼¶¼¼ÄÜ
		the_lvl = 90
	elseif (HaveMagic(1360) >= 0) then		-- ÓĞÕòÅÉ¾øÑ§¼¼ÄÜ
		the_lvl = 70
	elseif (UTask >= 60*256) then		-- ÒÑÍê³É50¼¶ÈÎÎñ
		the_lvl = 60
	elseif (UTask >= 50*256) then		-- ÒÑÍê³É40¼¶ÈÎÎñ
	...
	del_all_skill()
	add_hs(the_lvl)			-- ¼Ó»ØÓ¦Ñ§»áµÄ¼¼ÄÜ
	SetTask(137,30)
end

function update_wh()			-- Ë¢ĞÂ¼¼ÄÜ
	UTask = GetTask(4406)
	if (GetTask(4407) == 255) then		-- Ñ§»áÁË90¼¶¼¼ÄÜ
		the_lvl = 90
	elseif (HaveMagic(1971) >= 0) then		-- ÓĞÕòÅÉ¾øÑ§¼¼ÄÜ
		the_lvl = 70
	elseif (UTask >= 60*256) then		-- ÒÑÍê³É50¼¶ÈÎÎñ
		the_lvl = 60
	elseif (UTask >= 50*256) then		-- ÒÑÍê³É40¼¶ÈÎÎñ
	...
	del_all_skill()
	add_wh(the_lvl)			-- ¼Ó»ØÓ¦Ñ§»áµÄ¼¼ÄÜ
	SetTask(137,31)
end

function update_xy()			-- Ë¢ĞÂ¼¼ÄÜ
	UTask = GetTask(4464)
	if (GetTask(4465) == 255) then		-- Ñ§»áÁË90¼¶¼¼ÄÜ
		the_lvl = 90
	elseif (HaveMagic(1890) >= 0) then		-- ÓĞÕòÅÉ¾øÑ§¼¼ÄÜ
		the_lvl = 70
	elseif (UTask >= 60*256) then		-- ÒÑÍê³É50¼¶ÈÎÎñ
		the_lvl = 60
	elseif (UTask >= 50*256) then		-- ÒÑÍê³É40¼¶ÈÎÎñ
...
function add_hs(lvl)			-- ¸ù¾İÈÎÎñµÈ¼¶¼ÓÏàÓ¦µÄ»ªÉ½¼¼ÄÜ
	...
	if (lvl >= 90) then		
		if (HaveMagic(1364) <= 0) then AddMagic(1364,20) end		--°ÁÑ©Ğ¥·ç[372]£¨90¼¶£©
		if (HaveMagic(1382) <= 0) then AddMagic(1382,20) end		--À×¶¯¾ÅÌì[375]£¨90¼¶£©
		if (HaveMagic(1365) <= 0) then AddMagic(1365,20) end		--ÃÅÅÉ120¼¼ÄÜ
	end
end

function add_wh(lvl)			-- ¸ù¾İÈÎÎñµÈ¼¶¼ÓÏàÓ¦µÄ»ªÉ½¼¼ÄÜ
	--AddMagic(210,1)
	if (lvl >= 10) then			-- ÒÑ¾­ÈëÃÅ£¨¼ÓÈëÃÅ¼¼ÄÜ£©
		if (HaveMagic(1972) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1972)		--°×ºç¹áÈÕ£¨ÈëÃÅ£©
		end
		if (HaveMagic(1974) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1974)		--Çå·çËÍË¬£¨ÈëÃÅ£©
		end
	end
	if (lvl >= 20) then		-- 10¼¶ÈÎÎñ£¨¼Ó10¼¶¼¼ÄÜ£©
		if (HaveMagic(1964) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1964)		--½£×Ú×Ü¾ö£¨10¼¶£©
		end
		if (HaveMagic(1975) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1975)		--ÁúÈÅÉí£¨10¼¶£©
		end
	end
	if (lvl >= 30) then		-- 20¼¶ÈÎÎñ£¨¼Ó20¼¶¼¼ÄÜ£©
		if (HaveMagic(1976) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1976)		--ÑøÎá½£·¨£¨20¼¶£©
		end
	end
	if (lvl >= 40) then		-- 30¼¶ÈÎÎñ£¨¼Ó30¼¶¼¼ÄÜ£©
		if (HaveMagic(1965) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1965)		--½ğÑãºá¿Õ£¨30¼¶£©
		end
		if (HaveMagic(1977) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1977)		--ÁúĞı½£Æø£¨30¼¶£©
		end
	end
	if (lvl >= 50) then		-- 40¼¶ÈÎÎñ£¨¼Ó40¼¶¼¼ÄÜ£©
		if (HaveMagic(1963) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1963)		--Ï£ÒÄ½£·¨£¨40¼¶£©
		end
		if (HaveMagic(1979) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1979)		--ÆøÕğÉ½ºÓ£¨40¼¶£©
		end
	end
	if (lvl >= 60) then		-- 50¼¶ÈÎÎñ£¨¼Ó50¼¶¼¼ÄÜ£©
		if (HaveMagic(1980) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1980)		--ÌìÉğµ¹Ğü£¨50¼¶£©
		end
	end
	if (lvl >= 70) then		-- »ØÊ¦ÈÎÎñ£¨¼ÓÕòÅÉ¾øÑ§£©
		if (HaveMagic(1982) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1982)		--»ÃÑÛÔÆäÎ£¨60¼¶£©
		end
		if (HaveMagic(1971) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1971)		--²ÔËÉÓ­¿Í£¨60¼¶£©
		end
		if (HaveMagic(1981) == -1) then
			AddMagic(1981);		--Ä¦ÔÆ½£Æø£¨60¼¶£©
		end
	end
	if (lvl >= 90) then		
		if (HaveMagic(1967) <= 0) then AddMagic(1967,20) end		--°ÁÑ©Ğ¥·ç[372]£¨90¼¶£©
		if (HaveMagic(1983) <= 0) then AddMagic(1983,20) end		--À×¶¯¾ÅÌì[375]£¨90¼¶£©
		if (HaveMagic(1984) <= 0) then AddMagic(1984,20) end		--ÃÅÅÉ120¼¼ÄÜ
	end
end

function add_xy(lvl)			-- ¸ù¾İÈÎÎñµÈ¼¶¼ÓÏàÓ¦µÄ»ªÉ½¼¼ÄÜ
	--AddMagic(210,1)
	if (lvl >= 10) then			
		if (HaveMagic(1904) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1904)		--°×ºç¹áÈÕ£¨ÈëÃÅ£©
		end
              
		if (HaveMagic(1881) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1881)		--°×ºç¹áÈÕ£¨ÈëÃÅ£©
		end
		if (HaveMagic(1905) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1905)		--Çå·çËÍË¬£¨ÈëÃÅ£©
		end
	end
	if (lvl >= 20) then		-- 10¼¶ÈÎÎñ£¨¼Ó10¼¶¼¼ÄÜ£©
		if (HaveMagic(1882) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1882)		--½£×Ú×Ü¾ö£¨10¼¶£©
		end
		if (HaveMagic(1906) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1906)		--ÁúÈÅÉí£¨10¼¶£©
		end
	end
	if (lvl >= 30) then		-- 20¼¶ÈÎÎñ£¨¼Ó20¼¶¼¼ÄÜ£©
		if (HaveMagic(1883) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1883)		--ÑøÎá½£·¨£¨20¼¶£©
		end
	end
	if (lvl >= 40) then		-- 30¼¶ÈÎÎñ£¨¼Ó30¼¶¼¼ÄÜ£©
		if (HaveMagic(1885) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1885)		--½ğÑãºá¿Õ£¨30¼¶£©
		end
		if (HaveMagic(1907) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1907)		--ÁúĞı½£Æø£¨30¼¶£©
		end
	end
	if (lvl >= 50) then		-- 40¼¶ÈÎÎñ£¨¼Ó40¼¶¼¼ÄÜ£©
		if (HaveMagic(1887) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1887)		--Ï£ÒÄ½£·¨£¨40¼¶£©
		end
		if (HaveMagic(1909) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1909)		--ÆøÕğÉ½ºÓ£¨40¼¶£©
		end
	end
	if (lvl >= 60) then		-- 50¼¶ÈÎÎñ£¨¼Ó50¼¶¼¼ÄÜ£©
		if (HaveMagic(1888) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1888)		--ÌìÉğµ¹Ğü£¨50¼¶£©
		end
	end
	if (lvl >= 70) then		-- »ØÊ¦ÈÎÎñ£¨¼ÓÕòÅÉ¾øÑ§£©
		if (HaveMagic(1890) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1890)		--»ÃÑÛÔÆäÎ£¨60¼¶£©
		end
		if (HaveMagic(1889) == -1) then		-- ±ØĞëÃ»ÓĞ¼¼ÄÜµÄ²Å¸ø¼¼ÄÜ
			AddMagic(1889)		--²ÔËÉÓ­¿Í£¨60¼¶£©
		end
	end
	if (lvl >= 90) then		
		if (HaveMagic(1891) <= 0) then AddMagic(1891,20) end		--°ÁÑ©Ğ¥·ç[372]£¨90¼¶£©
		if (HaveMagic(1910) <= 0) then AddMagic(1910,20) end		--À×¶¯¾ÅÌì[375]£¨90¼¶£©
		if (HaveMagic(1894) <= 0) then AddMagic(1894,20) end		--ÃÅÅÉ120¼¼ÄÜ
	end
end
...
function HoTroSkill()
	...
	if tbAllSkill[szFaction] == nil then
		Talk(1, "", "B¹n cÇn ph¶i gia nhËp m«n ph¸i tr­íc ®·.")
		return 
	end
	for i=1, 9 do
