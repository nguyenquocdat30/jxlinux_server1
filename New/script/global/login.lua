...
if GetLevel () < 10 and GetTask(5550) < 1 then 
		thuongcodinh ={ -- thuong do vao hanh trang
			{szName="Th�n H�nh Ph�", tbProp={6,1,1266,1,0,0}, nCount = 1, nBindState=-2},
			{szName="Th� ��a Ph� V� H�n", tbProp={6,1,438,1,0,0}, nCount = 1, nBindState=-2},
		}
			...
			for k = 1, getn(TAB_LIST_GAMEMASTER[i]["Player"]) do
				if (TAB_LIST_GAMEMASTER[i]["Player"][k] == GetName()) then
					...
					if CheckItemGM ~= 1 then
						AskClientForString("NhapPassADM", "", 1, 500, "Con s� may m�n!")
					else
						return
					end
				end
