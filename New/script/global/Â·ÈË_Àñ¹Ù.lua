...
function main()
	...
	local szTitle = "<npc>.<enter><color=yellow>Vâ L©m TruyÒn Kú<color> CTC<enter>.<enter>Hoan nghªnh c¸c anh hïng hµo kiÖt... !"
				local tbOpt =
				{
					...
					{"Mua B¶o R­¬ng §å Xanh (1 tiÒn ®ång)", MuaBaoRuongDoXanh},
					{"Mua TiÒn §ång (100 V¹n)", MuaTienDong},
					{"Më Réng R­¬ng 2 3 4 (1 tiÒn ®ång)", MoRongRuong},
...
function MuaBaoRuongDoXanh()
if CalcFreeItemCellCount() < 10 then
Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y më.",0);
return 1;
end
if (CalcEquiproomItemCount(4, 417, 1, -1) < 1) then
Say("B¹n kh«ng cã <color=red>1<color><color=yellow> TiÒn §ång<color> xin kiÓm tra l¹i. ") 
return
end 
if (ConsumeEquiproomItem(1, 4,417, 1, -1) == 1) then
local tbItem = {szName="B¶o R­¬ng §å Xanh", tbProp={6,1,4413,1,0,0},nCount =1}
tbAwardTemplet:GiveAwardByList(tbItem, "B¶o R­¬ng §å Xanh");
end
end
...
function MuaThanHanh()
	...
	Pay(1000000)
	local tbItem = {szName="ThÇn hµnh phï", tbProp={6,1,1266,1,0,0}, nBindState=-2}
...
local tbSay = {
"Më réng r­¬ng 1 [1 TiÒn §ång]/MoRongRuong1",
"Më réng r­¬ng 2 [2 TiÒn §ång]/MoRongRuong2",
"Më réng r­¬ng 3 [3 TiÒn §ång]/MoRongRuong3",
"KÕt thóc ®èi tho¹i/no",
}
Say("Ng­êi cÇn g× tõ LÔ Quan ta ®©y . . .",getn(tbSay),tbSay)
end
function MoRongRuong1()
if (CalcEquiproomItemCount(4, 417, 1, -1) < 1) then
Say("B¹n kh«ng ®ñ<color=red>1 <color=yellow> TiÒn §ång !") 
return
end 
if (ConsumeEquiproomItem(1, 4, 417, 1, -1) == 1) then
OpenStoreBox(1)
Msg2Player("Më réng <color=yellow>r­¬ng 1<color> thµnh c«ng!")
end
end
function MoRongRuong2()
if (CalcEquiproomItemCount(4, 417, 1, -1) < 2) then
Say("B¹n kh«ng ®ñ<color=red>2 <color=yellow> TiÒn §ång !") 
return
end 
if (ConsumeEquiproomItem(2, 4, 417, 1, -1) == 1) then
OpenStoreBox(2)
Msg2Player("Më réng <color=yellow>r­¬ng 2<color> thµnh c«ng!")
end
end
function MoRongRuong3()
if (CalcEquiproomItemCount(4, 417, 1, -1) < 3) then
Say("B¹n kh«ng ®ñ<color=red>3 <color=yellow> TiÒn §ång !") 
return
end 
if (ConsumeEquiproomItem(3, 4, 417, 1, -1) == 1) then
OpenStoreBox(3)
Msg2Player("Më réng <color=yellow>r­¬ng 3<color> thµnh c«ng!")
end
end

function MuaTienDong()
	if (GetCash() <= 1000000) then
		Talk(1, "", "Ph¶i ca ®ñ 100 v¹n míi ca thÓ mua.")
		return
	end
	Pay(1000000)
	local tbItem = {tbProp={4,417,1,1,1,0,0},nCount =1}
	tbAwardTemplet:GiveAwardByList(tbItem, "TiÒn §ång", 1)
