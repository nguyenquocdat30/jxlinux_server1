...
function getQueryPiece(nCount, bil_tb_GetInfo)
	local arytalk = 
	{
		"Thî rÌn: ChØ cã thÓ ®Æt vµo ®å phæ hoµng kim, vËt phÈm ng­¬i ®Æt vµo kh«ng ®óng!",
		"§Æt vµo ®å phæ Hoµng Kim/QueryPiece",
	...
	--------------------------------------------------------------------------------------------------------------------------------------------------------
	local nCurItemIdx = 0
	local nCurItemName = 0
	local nCurItemQuality = 0
	local tbCurItemProp = 0
	local nCurItemValue = 0;
	local nCurItemVer = 0
	
if bil_tb_GetInfo then
	nCurItemQuality = 0
	nCurItemVer = 4
	tbCurItemProp = {bil_tb_GetInfo[1], bil_tb_GetInfo[2], bil_tb_GetInfo[3]}
else
	if nCount ~= 1 then
		Say(arytalk[1],2,arytalk[2],arytalk[3])
		return
	end
	nCurItemIdx = GetGiveItemUnit(1);
	nCurItemName = GetItemName(nCurItemIdx);
	nCurItemQuality = GetItemQuality(nCurItemIdx);
	tbCurItemProp = pack(GetItemProp(nCurItemIdx));
	nCurItemValue = 0;
	nCurItemVer = ITEM_GetItemVersion( nCurItemIdx );
end
	--------------------------------------------------------------------------------------------------------------------------------------------------------
	if nCurItemQuality ~= 0 then
		Say(arytalk[1],2,arytalk[2],arytalk[3])
