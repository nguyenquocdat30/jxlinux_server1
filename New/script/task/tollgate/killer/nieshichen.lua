...
ContentList = {
	"<#>NÕu ai cã ®ñ 160 s¸t thñ gi¶n trong   tay th× giÊc méng trë thµnh ®Ö nhÊt s¸t thñ ®· xem nh­ hoµn thµnh. Mçi ngµy chØ cÇn hoµn thµnh <color=yellow>"..KillerBoss_Of_Day.." nhiÖm vô<color>lµ ®­îc. <enter>H·y tiÕp nhËn nhiÖm vô “th¸ch thøc thêi gian” ®Ó thÓ nghiÖm søc m¹nh ®ång ®éi vµ kh¶ n¨ng s¸t thñ cña m×nh.",
	...
	"<#> Ta ®Õn Mua S¸t Thñ Gi¶n CÊp 90/muastg",
...
function main()
	...
	if (Challengeoftime == 1 and KillerBoss == 1) then
		UWorld1082 = nt_getTask(1082);
		local tbDialog = {ContentList[24],ContentList[30],ContentList[17],ContentList[2],ContentList[3],ContentList[4],ContentList[5],ContentList[6],ContentList[7],ContentList[8],ContentList[9],ContentList[10], ContentList[31], ContentList[15]};
		Describe(DescLink_NieShiChen..ContentList[1], getn(tbDialog), unpack(tbDialog));
	elseif (Challengeoftime == 1 and KillerBoss == 0) then
		local tbDialog = {ContentList[24],ContentList[30], ContentList[15]};
		Describe(DescLink_NieShiChen..ContentList[1], getn(tbDialog), unpack(tbDialog));
	elseif (Challengeoftime == 0 and KillerBoss == 1) then
		local tbDialog = {ContentList[2],ContentList[3],ContentList[4],ContentList[5],ContentList[6],ContentList[7],ContentList[8],ContentList[9],ContentList[10],ContentList[15]};
		Describe(DescLink_NieShiChen..ContentList[1], getn(tbDialog), unpack(tbDialog));
	elseif (Challengeoftime == 0 and KillerBoss == 0) then
		local tbDialog = {ContentList[15]};
		Describe(DescLink_NieShiChen..ContentList[1], getn(tbDialog), unpack(tbDialog));
	end
end
...
function muastg()
	local a = GetCash()
	local b = CalcEquiproomItemCount(4, 417, 1, 1) ;
	if Buy_STG_Of_Coin ~= 1 then
		if a < Price_Cash then
			return Talk(1,"","Ng­êi Kh«ng §ñ <color=yellow>"..(Price_Cash/10000).."<color> V¹n L­îng")
		else
			Pay(Price_Cash)
			AddItem(6,1,400,90,random(0,4),0)
			Msg2Player("B¹n NhËn §­îc 1 S¸t Thñ Gi¶n CÊp 90")
		end
	elseif Buy_STG_Of_Coin ~= 0 then 
		if(b < Price_Coin) then
			return Talk(1,"","Trªn ng­êi §¹i HiÖp kh«ng cã ®ñ<color=yellow> "..Price_Coin.." TiÒn ®ång.")
		else
			ConsumeEquiproomItem(Price_Coin,4,417,1,1)
			AddItem(6,1,400,90,random(0,4),0)
			Msg2Player("B¹n NhËn §­îc 1 S¸t Thñ Gi¶n CÊp 90")
			return 
		end
