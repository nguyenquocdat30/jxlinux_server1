...
local tb = tbEquipTryOn:NewTemplate("Chïy")

tb.szFile_Male = "\\settings\\changefeature\\goldequip_male_chuy.txt"
tb.szFile_Female = "\\settings\\changefeature\\goldequip_female_chuy.txt"
...
function tb:CheckEquip(nItemIndex)
	...
	if nD ~= 0 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i Chïy.")
		return 
	end
	if nP ~= 4 then
		Talk(1, "", "Vò KhÝ nµy kh«ng ph¶i Chïy.")
