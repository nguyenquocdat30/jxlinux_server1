...
function main()
	...
	tbDailog:AddOptEntry("N©ng cÊp Trang BŞ TuyÖt §Ønh ThiÕu L©m", tbHKUpgrade.ThieuLam, {tbHKUpgrade})
	tbDailog:AddOptEntry("N©ng cÊp Ên TuyÖt §Ønh ThiÕu L©m", tbAnUpgrade.ThieuLam, {tbAnUpgrade})
...
function oldentence()
	...
	if (UTask_sl == 70*256) and (GetFaction() == "shaolin") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_sl == 70*256) and (GetTask(3) >= 5*256) and (GetTask(3) < 10*256) then
		SetTask(3,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_sl == 80*256) and (GetCamp() == 4) then
		SetTask(7,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (GetTask(3) == 70*256) and (GetTask(7) == 70*256) then
		SetTask(3,75*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
...
function common_talk()
	...
	elseif (GetSeries() == 0) and (GetFaction() == "shaolin") then
		if (UTask_sl == 10*256+10) and (HaveItem(26) == 1) then
			L10_prise()
		elseif ((UTask_sl == 10*256) and (GetLevel() >= 10)) then
		...
		elseif ((UTask_sl == 40*256) and (GetLevel() >= 40)) then
		...
		elseif (UTask_sl == 80*256) or (UTask_sl == 80) then
			check_skill()
		else
			Talk(1,"","Tèt! Tèt! Cè g¾ng lªn! PhËt Tæ sÏ phï hé cho ng­¬i ®ã.")
		end
	elseif (GetSeries() == 0) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_sl == 70*256) and ((GetTask(3) < 5*256) or (GetTask(3) == 75*256)) then
		Talk(1,"return_sele","S­ phô, ®Ö tö cã thÓ m­în kinh ®iÓn trong Tµng Kinh C¸c ®­îc kh«ng vËy?")
	elseif (GetCamp() == 4) and ((UTask_sl == 70*256+10) or (UTask_sl == 70*256+20)) then
...
function L40_sele3_correct1()
	SetTaskTemp(43,GetTaskTemp(43) + 1)
	L40_sele3_2()
end
