...
function main()
	...
	tbDailog:AddOptEntry("N©ng cÊp Trang BÞ TuyÖt §Ønh C«n L«n", tbHKUpgrade.ConLon, {tbHKUpgrade})
	tbDailog:AddOptEntry("N©ng cÊp Ên TuyÖt §Ønh C«n L«n", tbAnUpgrade.ConLon, {tbAnUpgrade})
...
function oldentence()
	...
	if (UTask_kl == 70*256) and (GetFaction() == "kunlun") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_kl == 70*256) and (GetTask(5) >= 5*256) and (GetTask(5) < 10*256) then
		SetTask(5,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (UTask_kl == 80*256) and (GetCamp() == 4) then
		SetTask(9,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
	elseif (GetTask(5) == 70*256) and (GetTask(9) == 70*256) then
		SetTask(5,75*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kÞp thêi håi phuc!",0)
		return
...
function common_talk()
	...
	elseif (GetSeries() == 4) and (GetFaction() == "kunlun") then
		if ((UTask_kl == 60*256+20) and (HaveItem(16) == 1)) then
		...
		elseif ((UTask_kl == 60*256) and (GetLevel() >=50)) then
		...
		elseif (UTask_kl == 80*256) then
			check_skill()
			Say("Cã vÎ nh­ vâ nghÖ cña ng­¬i còng kh«ng kÐm, cã thÝch xuèng nói thùc tiÔn mét phen?",2,"§Ö tö còng ®ang cã ý ®ã /goff_yes","§Ö tö muèn tËp luyÖn thªm 1 thêi gian n÷a. /no")
		else
	...
	elseif (GetSeries() == 4) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_kl == 70*256) and ((GetTask(5) < 5*256) or (GetTask(5) == 75*256)) then
		Talk(1,"return_select","S­ phô! §Ö tö ra giang hå phiªu b¹t ®· ®­îc mét thêi gian, c¶m thÊy m×nh kiÕn thøc kÐm cái, cã thÓ cho ®Ö tö quay vÒ t¸i nhËp m«n ®Ó tiÕn tu vâ c«ng?")
	elseif (GetCamp() == 4) and ((UTask_kl == 70*256+10) or (UTask_kl == 70*256+20)) then
		Say("§¹o lý lµm ng­êi nh­ ng­¬i võa nãi ®ã, häc ®­îc ë ®©u vËy?",2,"§· chuÈn bÞ xong/return_complete","VÉn ch­a chuÈn bÞ xong/return_uncompleted")
	elseif (GetTask(5) == 70*256+10) and (HaveMagic(182) >= 0) then
		Say("Vâ §ang toµn mét lò tham danh lõa g¹t mäi ng­êi, uæng c«ng ta tËn t©m lùc ®µo t¹o ng­¬i thÕ mµ ng­¬i d¸m lµm chuyÖn v« ¬n béi nghÜa nh­ vËy!",2,"Kh«ng sai, ý ta ®· quyÕt. /defection_yes","§Ö tö ®· biÕt lçi råi. /no")
	else
...
function check_skill()
	Say("Cã vÎ nh­ vâ nghÖ cña ng­¬i còng kh«ng kÐm, cã thÝch xuèng nói thùc tiÔn mét phen?",2,"D¹! §Ö tö ®i liÒn /goff_yes","§Ö tö tËp luyÖn vÉn ch­a ®ñ /no")
end
...
function return_select()
	Say("§iÒu nµy kh«ng hîp víi quy ®Þnh cña m«n ph¸i. Nh­ng nÕu ng­¬i lµ ng­êi thÊu hiÓu ®¹o lý, ta cã thÓ gióp ng­¬i nghÜ c¸ch ",2,"TÆng 50000 l­îng b¹c /return_yes","Kh«ng trïng ph¶n s­ m«n n÷a /return_no")
end

function return_yes()
	Talk(1,"","Tèt l¾m! Qu¶ nhiªn lµ ng­êi biÕt ®iÒu. Ng­¬i ®· ngé ra nh÷ng ®¹o lý lµm ng­êi råi, còng nªn nhanh chãng chuÈn bÞ ®i!")
	SetTask(9,70*256+20)
...
function return_uncompleted()
	Talk(1,"","Cø Êm Êm óng óng nh­ thÕ, lµm sao cho ng­êi ta tin ®­îc thµnh ý cña ng­¬i!")
end
