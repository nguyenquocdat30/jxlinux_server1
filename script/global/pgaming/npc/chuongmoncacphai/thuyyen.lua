...
function oldentence()
	...
	if (UTask_cy == 70*256) and (GetFaction() == "cuiyan") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_cy == 70*256) and (GetTask(1) >= 5*256) and (GetTask(1) < 10*256) then
		SetTask(1,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_cy == 80*256) and (GetCamp() == 4) then
		SetTask(6,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (GetTask(1) == 70*256) and (GetTask(6) == 70*256) then
		SetTask(1,75*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	...
	local tbDes = {
		"N©ng cÊp Trang BŞ TuyÖt §Ønh Thóy Yªn/#tbHKUpgrade:ThuyYen()",
		"N©ng cÊp Ên TuyÖt §Ønh Thóy Yªn/#tbAnUpgrade:ThuyYen()",
		"§¸i nghÖ ®Çu s­/#daiyitoushi_main(5)",
...
function common_talk()
	...
	elseif (GetSeries() == 2) and (GetFaction() == "cuiyan") then
		if (UTask_cy == 10*256+10) and (HaveItem(0) == 1) then
			Talk(2,"L10_prise","Ch­ëng m«n, ®Ö tö ®· gióp ng­êi t×m l¹i Thóy Vò Tr©m ®· mÊt! ","Tèt qu¸! Ng­¬i ®· t×m l¹i ®­îc råi.")
		elseif (UTask_cy == 10*256) and (GetLevel() >= 10) then
		...
		elseif (UTask_cy == 50*256+50) then
		...
		elseif (UTask_cy == 50*256) and (GetLevel() >= 50) then
		...
		elseif (UTask_cy == 60*256+30) then
			L60_prise()
		elseif (UTask_cy == 80*256) then
			check_skill()
	...
	elseif (GetSeries() == 2) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_cy == 70*256) and ((GetTask(1) < 5*256) or (GetTask(1) == 75*256)) then
	...
	elseif (GetCamp() == 4) and ((UTask_cy == 70*256+10) or (UTask_cy == 70*256+20)) then
...
function Uworld126_finish()
	if (GetTask(6) >= 70*256) and (GetTask(6) ~= 75*256) then
		...
		if (HaveMagic(336) == -1) then
			AddMagic(336,1)
		end
		if (HaveMagic(337) == -1) then
			AddMagic(337,1)
		end
