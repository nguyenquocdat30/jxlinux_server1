...
function main()
	...
	tbDailog:AddOptEntry("N©ng cÊp Trang BŞ TuyÖt §Ønh Nga My", tbHKUpgrade.NgaMy, {tbHKUpgrade})
	tbDailog:AddOptEntry("N©ng cÊp Ên TuyÖt §Ønh Nga My", tbAnUpgrade.NgaMy, {tbAnUpgrade})
...
function oldentence()
	...
	if (UTask_em == 70*256) and (GetFaction() == "emei") then
		SetFaction("")
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_em == 70*256) and (GetTask(6) >= 5*256) and (GetTask(6) < 10*256) then
		SetTask(6,0)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
	elseif (UTask_em == 80*256) and (GetCamp() == 4) then
		SetTask(1,70*256)
		Say("HÖ thèng ph¸t hiÖn sai sãt, ®· kŞp thêi håi phuc!",0)
		return
...
function common_talk()
	...
	elseif (GetSeries() == 2) and (GetFaction() == "emei") then
		if (UTask_em == 80*256) then
			check_skill()
		elseif ((UTask_em == 60*256+50) and (HaveItem(24) == 1)) then
			DelItem(24)
		...
		elseif (UTask_em == 60*256) and (GetLevel() >= 50) then
			DelItem(24)
		...
		elseif (UTask_em == 50*256+80) then
			L50_prise()
		...
		elseif (UTask_em == 50*256) and (GetLevel() >= 50) then
			DelItem(23)
		...
		elseif (UTask_em > 50*256) and (UTask_em < 60*256) then
			Talk(1,"","TÆng ®­îc 'Kim Cang Kinh' ch­a?")
		elseif (UTask_em > 60*256) and (UTask_em < 70*256) then
	...
	elseif (GetSeries() == 2) and (GetCamp() == 4) and (GetLevel() >= 60) and (UTask_em == 70*256) and (GetTask(6) < 5*256) then
	...
	elseif (GetCamp() == 4) and ((UTask_em == 70*256+10) or (UTask_em == 70*256+20)) then
	...
	elseif (UTask_em < 10*256) and (GetSeries() == 2) then
...
function defection_yes()
	n = 0
	i=80 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=101x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=77 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=79 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=81 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=82 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=83 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=84 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=85 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=86 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=87 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=88 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=89 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=91 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=93 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=252x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	i=92 x = HaveMagic(i) if (x ~= -1) then n = n + x end DelMagic(i)
	AddMagicPoint(n)
	SetTask(6,5*256+20)
	SetTask(1,75*256)
	SetRank(74)
...
function Uworld125_finish()
	...
	add_repute = ReturnRepute(30,100,4)
