...
local tb = tbEquipTryOn:NewTemplate("TÇm Xa - Phi §ao")

tb.szFile_Male = "\\settings\\changefeature\\goldequip_male_tamxa1.txt"
tb.szFile_Female = "\\settings\\changefeature\\goldequip_female_tamxa1.txt"
...
function tb:CheckEquip(nItemIndex)
	...
	if nD ~= 1 then
		Talk(1, "", "§©y kh«ng ph¶i vò khÝ Phi §ao.")
		return 
	end
	if nP ~= 1 then
		Talk(1, "", "§©y kh«ng ph¶i vò khÝ Phi §ao.")
