...
local nNum     = GetTask(ID_TASKLINK_LIMITNUM);
	...
	if nNowDate~=nOldDate then
		...
		SetTask(ID_TASKLINK_LIMITNUM_done, 0);
...
function main()
	...
	if (Seasonnpc ~= 1) then
		Talk(1,"","Tİnh N¨ng <color=yellow>D· TÈu<color> T¹m Thêi §ang §­îc §ãng Vui Lßng Quay L¹i Sau")
		return
	end
	
	if (GetLevel() < Seasonnpc_Lv) then
		Talk(1,"","Tİnh N¨ng <color=yellow>D· TÈu<color> Yªu CÇu CÊp <color=yellow>"..Seasonnpc_Lv.."<color> Míi Cã ThÓ NhËn NhiÖm Vô")
		return
	end
...
Say(myTitleText,
	...
	"Hoµn Thµnh Nhanh NhiÖm Vô!/hoanthanhdatau",
...
function hoanthanhdatau()
		
	local nNum_Done  = GetTask(ID_TASKLINK_LIMITNUM_done);
	
	if (CalcEquiproomItemCount(6,1,4385,-1) < 1) then
		return	Say("<color=green>§¹i HiÖp<color> Kh«ng Cã <color=yellow>Kim Bµi D· TÈu<color>")
	end
	if nNum_Done > 24 then
		return Say("H«m Nay B¹n §· Dïng 25 LÇn Hoµn Thµnh B»ng Kim Bµi D· TÈu Råi , Kh«ng ThÓ Sö Dông Thªm\n H·y §i Lµm NhiÖm Vô §Ó Cã ThÓ Hoµn Thµnh")
	end
	if (ConsumeEquiproomItem(1,6,1,4385,-1) == 1) then
		local nNum_Done  = GetTask(ID_TASKLINK_LIMITNUM_done);
		nNum_Done = nNum_Done + 1;
		SetTask(ID_TASKLINK_LIMITNUM_done, nNum_Done);
		Msg2Player("H«m Nay B¹n §· Dïng "..nNum_Done.."/25 Kim Bµi D· TÈu")
		Task_AwardRecord()
		Task_GiveAward()
	end
end
...
local AnBangId = {164,165,166,167} -- 20 »·½±Àø»Æ½ğ×°±¸µÄ±àºÅ
local nAnBangID = 0
local nGoldenID = 0
...
if (nTotalTask == 100) or (nTotalTask == 200) or (nTotalTask == 300) or (nTotalTask == 400) or (nTotalTask == 600) or (nTotalTask == 700) or (nTotalTask == 800) or (nTotalTask == 900) then
StackExp(20000000)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc 20 TriÖu §iÓm Kinh NghiÖm Céng Dån.")
end

if (nTotalTask == 1100) or (nTotalTask == 1200) or (nTotalTask == 1300) or (nTotalTask == 1400) or (nTotalTask == 1600) or (nTotalTask == 1700) or (nTotalTask == 1800) or (nTotalTask == 1900) then
StackExp(30000000)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc 30 TriÖu §iÓm Kinh NghiÖm Céng Dån.")
end

if (nTotalTask == 2100) or (nTotalTask == 2200) or (nTotalTask == 2300) or (nTotalTask == 2400) or (nTotalTask == 2600) or (nTotalTask == 2700) or (nTotalTask == 2800) or (nTotalTask == 2900) then
StackExp(40000000)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc 40 TriÖu §iÓm Kinh NghiÖm Céng Dån.")
end

if (nTotalTask == 3100) or (nTotalTask == 3200) or (nTotalTask == 3300) or (nTotalTask == 3400) or (nTotalTask == 3600) or (nTotalTask == 3700) or (nTotalTask == 3800) or (nTotalTask == 3900) then
StackExp(50000000)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc 50 TriÖu §iÓm Kinh NghiÖm Céng Dån.")
end

if (nTotalTask == 4100) or (nTotalTask == 4200) or (nTotalTask == 4300) or (nTotalTask == 4400) or (nTotalTask == 4600) or (nTotalTask == 4700) or (nTotalTask == 4800) or (nTotalTask == 4900) then
StackExp(60000000)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc 60 TriÖu §iÓm Kinh NghiÖm Céng Dån.")
end

if (nTotalTask == 500) or (nTotalTask == 1500) or (nTotalTask == 2500) or (nTotalTask == 3500) then
AddItem(6,1,26,0,0,0)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc <color=yewllow>Vâ L©m MËt TŞch<color>.")
end

if (nTotalTask == 1000) or (nTotalTask == 2000) or (nTotalTask == 3000) or (nTotalTask == 4000) then
AddItem(6,1,22,0,0,0)
Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc <color=yewllow>TÈy Tuû Kinh<color>.")
end

if (nTotalTask == 4500) or (nTotalTask == 5000) then
	local nAnBangID = AnBangId[random(getn(AnBangId))]
	AddGoldItem( 0, nAnBangID )
	AddGlobalCountNews("§¹i HiÖp <color=cyan>"..GetName().."<color>Hoµn Thµnh "..nTotalTask.." NhËn §­îc 1 Mãn Trang BŞ An Bang", 3);
	Msg2Player("B¹n ®· hoµn thµnh "..nTotalTask.." NhiÖm Vô NhËn §­îc <color=yewllow>1 Mãn Trang BŞ An Bang<color>.")
end
