...
function main()
	if YanDiQuest ~= 1 then
		return Talk(1,"","TÝnh N¨ng <color=yellow>Viªm §Õ B¶o Tµng<color> §ang §­îc T¹m §ãng Vui Lßng Quay L¹i Sau")
	end
...
function YDBZ_checkLimit2()
	...
	if (GetLevel() < YanDiQuest_Lv) then
		Say("CÊp bËc kh«ng ®ñ ®Ó tham gia ho¹t ®éng\nYªu CÇu §¼ng CÊp : <color=yellow>"..YanDiQuest_Lv.."<color>",0)
		return 1
	end
...
function YDBZ_checkLimit()
	...
	for i = 1, GetTeamSize() do 
		...
		--¼ì²éµÈ¼¶
		if(GetLevel() < YanDiQuest_Lv) then		--µÈ¼¶
			noplaylv = noplaylv + 1
...
--		end
	...
	if noplaylv > 0 then
		...
		szstr = szstr .. format("Ng­êi ch¬i %s trong ®éi vÉn ch­a ®¹t cÊp %s.",szstr1,YanDiQuest_Lv)
	end
	if noitem > 0 then
