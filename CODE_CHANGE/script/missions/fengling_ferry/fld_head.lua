...
function fld_wanttakeboat(addr)
	...
	if(GetTask(5999) == nNowDate) then
		if (GetTask(5998) <= 0) then
			Say("H«m nay L­ît Tham Gia Phong L¨ng §é Cña Quı §¹i HiÖp §· HÕt, Ngµy Mai H·y Quay L¹i .",0);
			return
		end
	end
	local MemberNowDate = tonumber(GetLocalDate("%y%m%d"));
	if(GetTask(5999) == MemberNowDate) then
		if(GetTask(5998) <= 0) then
			Say("H«m nay L­ît Tham Gia Phong L¨ng §é Cña Quı §¹i HiÖp §· HÕt, Ngµy Mai H·y Quay L¹i NhĞ",0);
			return
		end
	end
	...
	local sz_msg = "Muèn ®ãn thuyÒn ®Õn bê B¾c Phong L¨ng §é ph¶i cã Phong L¨ng §é lÖnh bµi, ta sÏ cho ng­¬i lªn thuyÒn!";
	local str = {	
