...
function MONEY2BUILDFUND_G_1(nTongID, nOffer)
	...
	Msg2Player("B¹n ®ãng gãp <color=gold>"..nOffer.."<color> l­îng vµo ng©n s¸ch kiÕn thiÕt bang, nhËn ®­îc <color=green>"..nContribution.."<color> ®iÓm cèng hiÕn")
	return 1
end
...
function MEMBER_ADD_R(nTongID, nMemberID, nOfferBring)
	...
	local b = TONGM_GetName(nTongID, nMemberID)
	...
	Msg2Tong(nTongID, "<color=fire>Tæng thµnh viªn bang héi hiÖn t¹i: "..a.."/".._SETTING_TONG_COUNT_MEMBER)
	------------------------------------------------- Limit memberCount Join Tong ------------------------------------------------------
...
function TONG_ADDMEMBER_CHECK_ALLOW(_TongID, _MemID, _)
	local a = TONG_GetMemberCount(_TongID, -1)
	
	if a > _SETTING_TONG_COUNT_MEMBER then
		Msg2Tong(_TongID, "<color=fire>Giíi h¹n thµnh viªn: V× sè l­îng thµnh viªn cña bang héi ®· ®¹t ®Õn giíi h¹n hiÖn t¹i ".._SETTING_TONG_COUNT_MEMBER.." ng­êi nªn kh«ng thÓ tiÕp nhËn thµnh viªn míi!")
		return 0
	end
...
function CONFIGURE_TONG_MAP_G(nTongID)
	...
	local szState = {[0]="<color=green>Cho phÐp<color>",
				[1]="<color=red>Ng¨n cÊm<color>"}
	if (ban ~= 0)then
		ban = 1
	end	
...
function CITY_OCCUPY_R(nOwner, nPreOwner, nCityMap)
	...
	if (TONG_GetBuildLevel(nPreOwner) >= 5) then
		...
		local szMsg="Bang héi bÞ mÊt thµnh, ®¼ng cÊp kiÕn thiÕt bang gi¶m xuèng cÊp 4"
